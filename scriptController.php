...
if ($type == "vkRepost")
{
    ...
    if ($error == 0)
    {
        ...
        mysqli_query($connection,$update_sql) or die("Ошибка вставки" . mysqli_error());
        $update_sql12 = "Update users set vkrep='1' WHERE hash='$sid'";
        mysqli_query($connection,$update_sql12) or die("Ошибка вставки" . mysqli_error());
        $fa = "success";
    }
    $result = array(
        'success' => "$fa",
...
if ($type == "vkBonus")
{
    ...
    if ($error == 0)
    {
        ...
        mysqli_query($connection,$update_sql1) or die("Ошибка вставки" . mysqli_error());
        $fa = "success";
    }
    $result = array(
        'success' => "$fa",
...
if ($type == "bonus")
{
    ...
    if ($time - $bonus > 86400)
    {
        if ($error == 0)
        {
            ...
            mysqli_query($connection,$update_sql) or die("Insertion error" . mysqli_error());
            $update_sql1 = "Update users set bdate='$time' WHERE hash='$sid'";
            mysqli_query($connection,$update_sql1) or die("Insertion error" . mysqli_error());
            $fa = "success";
        }
    }
    else
...
if ($type == "activePromo") {
	...
	if ($price == "") {
		...
		$result1 = mysqli_query($connection,$sql_select1) or die(mysqli_error());
		$row = mysqli_fetch_array($result1) or die(mysqli_error());
		if ($row) {
			$user_id = $row['id'];
			$balance = $row['balance'];
		...
		$result5 = mysqli_query($connection,$sql_select5) or die(mysqli_error());
