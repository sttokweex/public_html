<?php
include(dirname(__DIR__, 1) . "/system/config.php");
...
$getDepsForLevel = "SELECT SUM(amount) FROM deposits WHERE user_id='$idu' AND status='1'";
...
if (isset($_POST['mess'])) {
    ...
    if ($ban != 1) {
        ...
        if ($depositesSID >= 500) {
            ...
            $obj = array(
                "good" => "false",
                "mess" => "Чат доступен игрокам пополнившим более 500 монет за все время!"
            );
        }
    } else {
        $obj = array(
            "good" => "false",
            "mess" => "Вы заблокированы в чате"
        );
    }
}
if (isset($_POST['del'])) {
