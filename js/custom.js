...
window.t = (k, fb = '') => (window.T && Object.prototype.hasOwnProperty.call(window.T, k) ? window.T[k] : (fb || k));

fetch('../lang/translations.php')
    .then(r => r.json())
    .then(tr => { window.T = tr; console.log('i18n loaded'); })
    .catch(err => { console.warn('i18n load failed', err); window.T = {}; });
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (obj.success == "success") {
                $('#rbbtn').html(T.take);
                $('#rakebackval').html('0');
                toastr['success'](`${T.enrolled} Rakeback: <b>` + obj.rakebacksize + "</b>")
                $('#userBalance').html(obj.new_balance);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (obj.success == "success") {
                $('#csbtn').html(T.take);
                $('#cashbackval').html('0');
                toastr['success'](`${T.enrolled} Cashback: <b>` + obj.cashbacksize + "</b>")
                $('#userBalance').html(obj.new_balance);
                updateBalance(obj.balance, obj.new_balance);
            } else {
                $('#csbtn').html(T.take);
...
function loadGreen() {
    $('#x2wrap').css("background", "linear-gradient(90deg, rgb(27, 147, 30), rgb(71, 183, 65))");
    $('#x3wrap').css("background", "linear-gradient(90deg, rgb(27, 147, 30), rgb(71, 183, 65))");
    $('#x4wrap').css("background", "linear-gradient(90deg, rgb(27, 147, 30), rgb(71, 183, 65))");
    $('#x5wrap').css("background", "linear-gradient(90deg, rgb(27, 147, 30), rgb(71, 183, 65))");
}
...
function loadRed() {
    $('#x2wrap').css("background", "linear-gradient(90deg, rgb(209, 33, 33), rgb(231, 86, 86))");
    $('#x3wrap').css("background", "linear-gradient(90deg, rgb(209, 33, 33), rgb(231, 86, 86))");
    $('#x4wrap').css("background", "linear-gradient(90deg, rgb(209, 33, 33), rgb(231, 86, 86))");
    $('#x5wrap').css("background", "linear-gradient(90deg, rgb(209, 33, 33), rgb(231, 86, 86))");
}
...
function minwithsum() {
    ...
    if (inp < mininal || inp > 1000000) {
        ...
        $('#withdrawSizeAlert').html(`${T.amount_from} <b>` + mininal + `</b> ${T.up} <b>1000000</b>`);
...
function minwithnumber() {
    ...
    if (inp.length < 5) {
        ...
        $('#walletNumberAlert').html(`${T.length_of_wallet_from} <b>5</b> ${T.characters}`);
        $('#withBtn').attr("disabled", true);
        $('#withBtn').css("opacity", "0.5");
    } else {
